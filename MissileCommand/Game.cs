...
namespace MissileCommand
{
    ...
    public class Game : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            _titlescreen = Content.Load<Texture2D>("basic game picture");
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (_state == GameState.MenuScreen)
            {
                updateMenuScreen(gameTime);
            }
            else if (_state == GameState.MultiplayerWaitingScreen)
            {
                updateMultiplayerWaitingScreen(gameTime);
            }
            else if (_state == GameState.MultiplayerRunningScreen)
            {
                updateMultipleRunningScreen(gameTime);
            }
        ...
        private void updateMenuScreen(GameTime gameTime)
        {
            if (_keyState.isPressed(Keys.C))
            {
                ...
                _state = GameState.MultiplayerWaitingScreen;
            }
        ...
        private void updateMultiplayerWaitingScreen(GameTime gameTime)
        {
            if (_networkManager.role == NetRole.Client)
            {
                if (!_networkManager.connected)
                {
                    // loops to get keybaord input, every letter
                    for (int i = 0; i < 10; i++)
                    {
                        Keys key = (Keys)(i + 48);
                    ...
                    if (_keyState.isPressed(Keys.Enter) && _oldKeyState.isReleased(Keys.Enter))
                    {
                        ...
                        _networkManager.connectTo(_ipAddressValue); // connect to server
        ...
        private void updateMultipleRunningScreen(GameTime gameTime)
        {
            processInput();
        ...
        public void startGame()
        {
            ...
            _state = GameState.MultiplayerRunningScreen;
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            {
                drawMultiplayerWaitingScreen(gameTime);

            }
            else if (_state == GameState.MultiplayerRunningScreen)
            {
                drawMultiplayerRunningScreen(gameTime);
        ...
        private void drawMenuScreen(GameTime gameTime)
        {
            ...
            spriteBatch.DrawString(_font, "Multiplayer Missile Command", new Vector2(350, 100), Color.White, 0, new Vector2(0, 0), 2f, 0, 0);

            spriteBatch.DrawString(_font, "Press C to start the game as the Client ", new Vector2(100, 300), Color.White);
            spriteBatch.DrawString(_font, "Press S to Host the game as the Server", new Vector2(100, 400), Color.White);
            spriteBatch.End();
        }

        private void drawMultiplayerWaitingScreen(GameTime gameTime)
        {
            GraphicsDevice.Clear(Color.Black);
            if (_networkManager.role == NetRole.Client)
            {
        ...
        private void drawMultiplayerRunningScreen(GameTime gameTime)
        {
            GraphicsDevice.Clear(Color.Black);
            spriteBatch.Begin();
